...
d3.chart.dependencyWheel = function(options) {
  ...
  function chart(selection) {
    ...
    //console.log("+++++ chart depency weel +++++", selection)

    if (document.querySelector("svg") !== null) {
      //console.log("remove existing svg element")
    ...
    selection.each(function(data) {
      ...
      const NO_PACKAGE_NAME = "Empty string"
      ...
      //console.log("width:", width, svg)

      // Otherwise, create the skeletal chart.
      var gEnter = svg.enter().append("svg:svg")
        //.attr("width", width)
        //.attr("height", width)
        .attr("class", DEPENDENCY_WEEL_CLASS_NAME)
      ...
      var fill = function(d) {
        ...
        let nodeName = packageNames[d.index]
        if (nodeName === "") {
          nodeName = NO_PACKAGE_NAME
        }
        //console.log('filll', nodeName ,nodeName.charCodeAt())
        return "hsl(" + parseInt(((nodeName.charCodeAt(0) - 97) / 26) * 360, 10) + ",90%,70%)";
      };
      ...
      g.append("svg:text")
        ...
        .style("cursor", "pointer")
        .text((d) => packageNames[d.index]===""?NO_PACKAGE_NAME:packageNames[d.index] ) // set label
        .on("mouseover", fade(0.1))
        .on("mouseout", fade(1));
